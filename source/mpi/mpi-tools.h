...
namespace mpi {
    ...
    [[nodiscard]] constexpr MPI_Datatype get_dtype() noexcept {
        ...
        if constexpr(tc::has_Scalar_v<D>) return get_dtype<typename D::Scalar>();
        if constexpr(tc::has_value_type_v<D>) return get_dtype<typename D::value_type>();
