...
namespace tools::h5io {
    namespace internal {
        ...
        void copy_dset(h5pp::File &h5_tgt, const h5pp::File &h5_src, h5pp::DsetInfo &tgtInfo, h5pp::DsetInfo &srcInfo, long index) {
            ...
            if (uindex >= tgtInfo.dsetDims.value().at(1))
                return append_dset<T>(h5_tgt,h5_src,tgtInfo,srcInfo);
    ...
    std::string get_standardized_base(const ModelId<T> &H, int decimals) {
        if constexpr (std::is_same_v<T,sdual>)
            return h5pp::format("L_{1}/l_{2:.{0}f}/d_{3:+.{0}f}", decimals, H.model_size, H.p.lambda, H.p.delta);
    ...
    std::vector<std::string> findKeys(const h5pp::File &h5_src, const std::string &root, const std::vector<std::string> &expectedKeys, long hits, long depth) {
        ...
        for(const auto &key : expectedKeys) {
            ...
            bool                   cacheHit    =
                                            cachedIt != cache.end() and   // The search query has been processed earlier
                    (
    ...
    template<typename T>
    void saveModel(const h5pp::File &h5_src, h5pp::File &h5_tgt, std::unordered_map<std::string, InfoId<h5pp::TableInfo>> &tgtModelDb, const ModelId<T> &modelId) {
        tools::prof::t_ham.tic();
    ...
    template void saveModel(const h5pp::File &h5_src, h5pp::File &h5_tgt, std::unordered_map<std::string, InfoId<h5pp::TableInfo>> &tgtModelDb, const ModelId<sdual> &modelId);
    template void saveModel(const h5pp::File &h5_src, h5pp::File &h5_tgt, std::unordered_map<std::string, InfoId<h5pp::TableInfo>> &tgtModelDb, const ModelId<lbit> &modelId);
    ...
    void transferCronos(h5pp::File &h5_tgt, std::unordered_map<std::string, InfoId<h5pp::TableInfo>> &tgtTableDb,
        ...
        for(const auto &srcKey : srcTableKeys) {
            ...
            for(size_t rec = 0; rec < srcRecords; rec++){
                ...
                try{
                    h5pp::hdf5::readTableField(iter,srcInfo, {"iter"},rec,1);
                }catch(const std::exception & ex) {
                    tools::logger::log->warn("Failed to get iteration number: {}", ex.what());
                    iter = rec;
                }
//                if(iter != rec) tools::logger::log->warn("iter {} != rec {}", iter,rec);
                auto  tgtName = h5pp::format("{}_{}",h5pp::fs::path(srcInfo.tablePath.value()).filename().string(), iter);
                auto  tgtPath = h5pp::format("{}/iter/{}", groupPath, tgtName);
                tools::prof::t_crt.tic();
                try{
                    if(tgtTableDb.find(tgtPath) == tgtTableDb.end()) {
                        tools::logger::log->info("Adding target table {}", tgtPath);
