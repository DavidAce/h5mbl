...
namespace tools::io {
    ...
    std::vector<h5pp::fs::path> find_file(const h5pp::fs::path &base, const std::regex &pattern) {
        ...
        for(auto &obj : dir_iterator(base))
            if(h5pp::fs::is_regular_file(obj) and std::regex_match(obj.path().filename().string(), pattern)) result.emplace_back(obj);
        return result;
    }
    template std::vector<h5pp::fs::path> find_file<true>(const h5pp::fs::path &base, const std::regex &pattern);
    template std::vector<h5pp::fs::path> find_file<false>(const h5pp::fs::path &base, const std::regex &pattern);
    ...
    std::vector<h5pp::fs::path> find_dir(const h5pp::fs::path &base, const std::string &pattern, const std::string &subdir) {
        ...
        for(const auto &obj : dir_iterator(base)) {
            ...
            if(h5pp::fs::is_directory(obj.path()) and match) {
                if(h5pp::fs::exists(obj.path() / "output")) result.emplace_back(h5pp::fs::canonical(obj.path() / "output"));
            }
        }
