...
namespace num {
    ...
    auto sum(const Input &in, std::optional<long> from = std::nullopt, std::optional<long> num = std::nullopt) {
        if(not from) from = 0;
        if(not num) num = in.size();
        return std::accumulate(in.data() + from.value(), in.data() + from.value() + num.value(), static_cast<typename Input::value_type>(0));
    }
